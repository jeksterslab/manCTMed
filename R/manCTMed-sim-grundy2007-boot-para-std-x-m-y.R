#' Simulation Replication - Grundy2007BootParaStdXMY
#'
...
SimGrundy2007BootParaStdXMY <- function(taskid,
                                        repid,
                                        output_folder,
                                        seed,
                                        suffix,
  ...
  fn_output <- SimFN(
    output_type = "grundy2007-dynr-boot-para-xmy",
    output_folder = output_folder,
    suffix = suffix
  )
  run <- .SimCheck(
  ...
  if (run) {
    ...
    con <- file(fn_output)
    saveRDS(
      object = Grundy2007BootParaStdXMY(
        boot = readRDS(fn_boot),
        theta_hat = ThetaHat(readRDS(fn_fit)),
        delta_t = delta_t,
        ncores = NULL
      ),
      file = con
