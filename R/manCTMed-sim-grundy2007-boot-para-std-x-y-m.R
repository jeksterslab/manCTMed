#' Simulation Replication - Grundy2007BootParaStdXYM
#'
...
SimGrundy2007BootParaStdXYM <- function(taskid,
                                        repid,
                                        output_folder,
                                        seed,
                                        suffix,
  ...
  fn_output <- SimFN(
    output_type = "grundy2007-dynr-pb-std-xym",
    output_folder = output_folder,
    suffix = suffix
  )
  run <- .SimCheck(
  ...
  if (run) {
    ...
    con <- file(fn_output)
    saveRDS(
      object = Grundy2007BootParaStdXYM(
