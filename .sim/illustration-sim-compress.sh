...
#SBATCH --output=i-com.out
#SBATCH --error=i-com.err
...
PARALLEL_TMP_FOLDER=$(mktemp -d -q "/scratch/$USER/${PROJECT}/.sim/tmp/$TODAY-i-com-XXXXXXXX")
...
cmd="apptainer exec \
     ...
     Rscript /scratch/\$USER/${PROJECT}/.sim/illustration-sim-compress.R {1} {2}; \
...
# done -------------------------------------------------------------------------
echo "illustration-sim-compress.sh done"
# ------------------------------------------------------------------------------
