data_process_example_data <- function(overwrite = FALSE,
                                      n) {
  cat("\ndata_process_example_data\n")
  # find root directory
  ...
  if (!file.exists(grundy2007_file)) {
    write <- TRUE
  } else {
    if (overwrite) {
      write <- TRUE
  ...
  if (write) {
    ...
    save(
      grundy2007,
      file = grundy2007_file,
      compress = "xz"
    )
  }
}
data_process_example_data(n = 133)
rm(data_process_example_data)
