data_process_empirical_mislabeled_med <- function(overwrite = FALSE) {
  cat("\ndata_process_empirical_mislabeled_med\n")
  set.seed(42)
  # find root directory
  ...
  source(
    root$find_file(
      ".setup",
      "data-process",
      "data-process-empirical-ct-mislabeled.R"
    )
  )
  fit_empirical_mislabeled <- root$find_file(
    ".setup",
    "data-raw",
  ...
  med_file <- root$find_file(
    ".setup",
    "data-raw",
    "med-empirical-mislabeled.Rds"
  )
  ci_file <- root$find_file(
    ".setup",
    "data-raw",
    "ci-empirical-mislabeled.Rds"
  )
  ci_beta_file <- root$find_file(
    ".setup",
    "data-raw",
    "ci-empirical-mislabeled-beta.Rds"
  )
  ...
  empirical_mislabeled_table_coef <- root$find_file(
    ".setup",
    "data-raw",
    "empirical-mislabeled-table-coef.Rds"
  )
  if (
    !all(
      file.exists(
        c(
          ...
          ci_file,
          ci_beta_file,
          empirical_mislabeled_table_coef
        )
      )
    )
  ) {
  ...
  if (write) {
    ...
    fit_summary <- readRDS(fit_empirical_ct_mislabeled_summary)$Coefficients
    phi <- cbind(
    ...
    saveRDS(
      coefs,
      file = empirical_mislabeled_table_coef,
      compress = "xz"
    )
  }
}
data_process_empirical_mislabeled_med()
rm(data_process_empirical_mislabeled_med)
