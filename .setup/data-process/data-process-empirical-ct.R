data_process_empirical_ct <- function(overwrite = FALSE) {
  cat("\ndata_process_empirical_ct\n")
  set.seed(42)
  # find root directory
  ...
  data_empirical_ct <- root$find_file(
    ".setup",
    "data-raw",
    "ESMdata.csv"
  )
  fit_empirical_ct <- root$find_file(
    ".setup",
    "data-raw",
  ...
  if (write) {
    ...
    saveRDS(
      fit,
      file = fit_empirical_ct,
      compress = "xz"
    )
    saveRDS(
      summary(fit),
      file = fit_empirical_ct_summary,
      compress = "xz"
    )
  }
}
data_process_empirical_ct()
rm(data_process_empirical_ct)
